 ...
 * on this device. Every transition — including signing out — goes through handleAuthChange, which
 * purges whichever identity's cloud-owned data this device's cache no longer belongs to (see its
 * own comment in useAccountsStore.ts); a signed-out broker's genuinely local-only (never-synced)
 * submissions are left untouched, so the "always works, even signed out" experience still holds.
 */
function useBrokerCloudBootstrap() {
  const handleAuthChange = useAccountsStore((s) => s.handleAuthChange);
