...
-- on coverage_lines.coverage_type, the same pattern 0006_widen_extraction_method_check.sql used for
-- extraction_method. Safe to run against a live project with existing coverage_lines rows: every
-- row already there satisfies the OLD (narrower) list, which is a subset of the new one.
--
...
--
-- Constraint name is looked up dynamically for the same reason 0006 did: this runs correctly
